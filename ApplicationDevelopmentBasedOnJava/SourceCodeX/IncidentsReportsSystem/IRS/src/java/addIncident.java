 ...
 * @author JakeLin
 */
@WebServlet(urlPatterns = {"/addIncident"})
public class addIncident extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            incident.roughLocation = request.getParameter("loc");
            incident.latlng = request.getParameter("lat") + "," + request.getParameter("lng");
            incident.reportedby = Integer.parseInt(request.getParameter("l2admin"));
            
            IncidentsManager incidentsManager = new IncidentsManager();
            if(true == incidentsManager.addIncident(incident)){
                out.println("OK");
            }else{
             out.println("<ERROR");                
            }
